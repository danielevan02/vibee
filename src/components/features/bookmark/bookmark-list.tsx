...
import PostCard, { type PostCardProps } from "@/components/features/post/post-card";
...
/**
 * Client island for the saved list. It only exists because PostCard needs the
 * browser (likes, bookmarks, menus); the data itself arrives as a prop already
 * resolved on the server, so there is no fetch-on-mount and no spinner.
 */
export default function BookmarkList({ posts }: BookmarkListProps) {
  return (
